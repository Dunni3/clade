...
Claude Code is installed on all containers at build time, but authentication must be done manually after each rebuild. Attach to a container and run `claude login` — it will print a URL to open in your browser to complete OAuth.
...
- **No MCP registration testing**: `--no-mcp` skips writing `~/.claude.json`. Testing MCP registration requires an authenticated Claude Code session.
