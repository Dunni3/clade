...
from .ember_setup import setup_ember, setup_sudoers
...
@click.option("--sudoers", "setup_sudoers_flag", is_flag=True, help="Set up passwordless sudo for Ember service restarts")
@click.option("--yes", "-y", is_flag=True, help="Accept defaults without prompting")
@click.pass_context
def setup_ember_cmd(ctx: click.Context, name: str, port: int, setup_sudoers_flag: bool, yes: bool) -> None:
    ...
    # Sudoers setup
    if setup_sudoers_flag:
        if setup_sudoers(bro.ssh):
            bro.sudoers_configured = True
            save_clade_config(config, config_path)
