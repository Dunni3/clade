...
async def get_tree(root_task_id: int) -> dict | None:
    ...
    try:
        # Fetch root task
        cursor = await db.execute(
            "SELECT id, creator, assignee, subject, status, created_at, started_at, completed_at, parent_task_id, root_task_id, prompt, session_name, host, working_dir, output, blocked_by_task_id FROM tasks WHERE id = ?",
            (root_task_id,),
        )
        ...
        cursor = await db.execute(
            "SELECT id, creator, assignee, subject, status, created_at, started_at, completed_at, parent_task_id, root_task_id, prompt, session_name, host, working_dir, output, blocked_by_task_id FROM tasks WHERE root_task_id = ? AND id != ? ORDER BY created_at ASC",
